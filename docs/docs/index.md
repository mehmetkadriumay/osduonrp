...
!!! example "CI Butler is Not Yet Using Official Community Implementation Helm Charts and Images Yet"
    ...
    These versions and paths can however be easily overridden on the command-line.
    Once stable community provided charts and images are available, CI Butler will be updated to use these by default. This team is making great progress so it shouldn't be too long now.

!!! tip "Automation and Testing"
    CI Butler also has built-in support for managing users and client ids in keycloak and adding users to groups OSDU.

    This can be especially useful to when you need to redeploy your OSDU environment for testing or as part of a pipeline.
